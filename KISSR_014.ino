 ...
 * Version History:                                                  
 * KISSR_014    (Dec 2017) 
 * - Implement LED blink via interrupt 
 * KISSR_013    (Dec 2017) 
 * - Correction to BMP180 pressure calculation
 * - Calculate ref pressure in setup
 * - Low power sleep enabled - still not sure if delay is needed though
 * - Reworked variable definitions, more consistant use of uint16_t variable types
 * - ADXL code removed
 * - Change from wire.h library to I2C.h library, as it allows timeout if bus is not available
 * - Implemented LED heartbeat --> to be updated to intrrupt in due course
 * KISSR_012    (Dec 2017)
 * - Watchdog interrupt enabled - requires update of bootloader to work
 * - Simplify debug outputs, make output consistent with excel macro
 * - Better logging of known issues and things to do (worklist)
 * KISSR_011    (Dec 2017)
 * - Working version after late 2017 restart
 * - Log fewer errors
 *=============================================================================
...
//Includes
//-----------------------------------------------------------------------------
//  #include     <Wire.h>               // library for communicating over i2c protocol (default arduino library)
  #include     <I2C.h>                  // library for communicating over i2c protocol see http://dsscircuits.com/articles/arduino-i2c-master-library
  #include     <OneWire.h>               
  #include     <DallasTemperature.h>  
  #include     <TinyGPS++.h>            // library for controlling gps module
  #include     <SoftwareSerial.h>       // software serial emulator 
...
//-----------------------------------------------------------------------------
  #define DEBUG          0       // reports code position for detailed debugging
